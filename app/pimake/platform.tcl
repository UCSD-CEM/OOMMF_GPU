...
#    handle unmatched open brace as an element.  The replacement is safe.
    ...
    proc LinkerLibs {stemlist} {
        ...
        foreach stem $stemlist {
			...
			if {[string match mkl $stem]} {
				set ret [concat $ret [$configuration GetValue MKL_LIB]]
				#set ret [concat $ret [$configuration GetValue MKL_INTEL_LIB]]
				#set ret [concat $ret [$configuration GetValue MKL_THREADED_LIB]]
				continue
			}
			#cuiwl: set the omp libs
			if {[string match omp $stem]} {
				set ret [concat $ret [$configuration GetValue OMP_LIB]]
				continue
			}
			#cuiwl: set the cuda libs
			if {[string match cu $stem]} {
				#set ret [concat $ret [$configuration GetValue CUDADEVRT_LIB]]
				set ret [concat $ret [$configuration GetValue CUDA_LIB]]
				#set ret [concat $ret [$configuration GetValue CUDART_LIB]]
				#set ret [concat $ret [$configuration GetValue CUFFT_LIB]]
				continue
			}
    ...
    proc Compile {language args} {
		...
		# compcmd set to cl for c++ compiling in windows-x86_64 and nvcc for cuda compiling
		...
		if {[string match CUDA $language]} {
			set args [concat $cuflags $args]
		}
		
		#cuiwl: for nvcc src files need to be identified first
		if {[string match CUDA $language]} {
			    set cusrc [lindex $args end]
				set args [lrange $args 0 end-2]
				#lappend compcmd $cusrc
			}
        ...
        while {[string match -* [lindex $args 0]]} {
			...
			if {[string match out $opt] && [string match CUDA $language]} {
				lappend compcmd $cusrc
			}
			
			#cuiwl: special case for CUDA since the platformName below won't match
			if {[string match CUDA $language]} {
				if {[catch {
					...
					set cuopt -${opt}
					foreach cuval $val {
						set cucmdflags ${cuopt}=${cuval}
						lappend compcmd $cucmdflags
					}
				}	
			}
    ...
    proc Link {args} {
        ...
        while {[string match -* [lindex $args 0]]} {
            regexp ^-(.*)$ [lindex $args 0] _ opt
			#cuiwl: special case for cuda device link
			if {[string match device $opt]} {
				set linkcmd [$configuration GetValue device_linker]
				set val [lindex $args 1]
				foreach cuobj $val {
					lappend linkcmd "windows-x86_64\\$cuobj.obj"
				}
				lappend linkcmd "-o"
				lappend linkcmd "windows-x86_64\\device.obj"
				#eval Oc_Exec Foreground $linkcmd
				set args [lrange $args 2 end]
				set linkcmd [$configuration GetValue program_linker]
				continue
			}
