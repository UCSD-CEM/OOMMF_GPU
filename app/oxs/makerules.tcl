...
if {[info exist env(CUDA_HOME)]} {
	set extlibs [concat $extlibs cu]
}
if {[info exist env(MKL_HOME)]} {
	#set extlibs [concat $extlibs mkl]
}
...
MakeRule Define {
    ...
    -script		[format {
			    Platform Link -obj {%s %s %s %s extinit} \
			            -lib {%s tk %s} \
			            -sub CONSOLE -out oxs
			} $objects $extobjs $lclobjs $lclcuobjs $oxslibs $extlibs]
}
...
#cuiwl: try if cuda codes can be complied in this way
...
#cuiwl: -I list
    ...
    if {[info exist env(CUDA_HOME)]} {
		...
		set path [concat $path $cuda_inc]
    }
    puts "CUDA_HOME=$cuda_home"
    set mkl_home ""
    if {[info exist env(MKL_HOME)]} {
        set mkl_home $env(MKL_HOME)
		set mkl_inc "{$mkl_home/include}"
		set path [concat $path $mkl_inc]
    }
    puts "MKL_HOME=$mkl_home"
foreach src $lclcusrcs {
...
MakeRule Define {
    ...
    } $objects $extobjs $lclobjs $pobjs $lclcuobjs\
