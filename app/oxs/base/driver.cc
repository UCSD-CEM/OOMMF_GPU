...
// GPU time driver interface
Oxs_Driver::Oxs_Driver
( const char* name,        // Child instance id
  Oxs_Director* newdtr,    // App director
  const char* argstr,       // Args
  OC_REAL8m &scaling_aveM_copy,
  OC_BOOL &normalize_aveM_copy
  ) : Oxs_Ext(name,newdtr,argstr),
      report_max_spin_angle(0),normalize_aveM(1),scaling_aveM(1.0),
      number_of_stages(1),
      checkpoint_id(0),checkpoint_writes(0),checkpoint_interval(0.),
      ...
      start_last_timestep(0.)
{
  ...
  normalize_aveM_copy = normalize_aveM;
  
  if(normalize_aveM) {
    const OC_INDEX mesh_size = Ms.Size();
    OC_REAL8m sum = 0.0;
    for(OC_INDEX j=0;j<mesh_size;++j) sum += fabs(Ms[j]);
    if(sum>0.0) scaling_aveM_copy = 1.0/sum;
    else        scaling_aveM_copy = 1.0; // Safety
  } else {
    if(Ms.Size()>0) scaling_aveM_copy = 1.0/static_cast<OC_REAL8m>(Ms.Size());
    else            scaling_aveM_copy = 1.0; // Safety
  }
