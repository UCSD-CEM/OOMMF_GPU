...
{ // Compute sums of energies, fields, and/or torques for all energies
  ...
  // USER HAVE NOT REQUIRE TO COMPUTE ANY ENERGY DURING COMPUTATION
  if(energies.size() == 0) {
  ...
  // DONT KNOW WHY THIS SITUATION WILL HAPPEN
  ...
  // DONT UNDERSTAND THIS PART...SEEMS TO DIVIDE ENERGY INTO TWO CATEGORIES: CHUNKENERGY AND
  // NON-CHUNKENERGY
  ...
  myCounter++;
	...
	// MOVES THE COTENTS BACK TO OCED FROM TERM_OCED
    if(!accums_initialized) {
...
#ifdef GPU_DEBUG
  FILE* debugInfo;
  debugInfo = fopen("log.txt", "a");
  fprintf(debugInfo, "non-chunk energy has %d members...\n", myCounter);
  fclose(debugInfo);
...
#ifdef GPU_DEBUG
  debugInfo = fopen("log.txt", "a");
  fprintf(debugInfo, "chunk energy has %d members...\n", myCounter);
  fclose(debugInfo);
#endif
