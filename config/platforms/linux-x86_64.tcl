...
$config SetValue program_compiler_c++ {g++ -g -c -lpthread}
#
...
#
#cuiwl: nvcc compiler
$config SetValue program_compiler_cuda {nvcc -c -w -m 64 -DNDEBUG -use_fast_math -Xcompiler -O2 -Xcompiler -ffast-math -Xcompiler -frename-registers -Xcompiler -fstrict-aliasing -Xcompiler -fweb -Xcompiler -fomit-frame-pointer}
...
#cuiwl: external libs are defined here
if {[info exist env(CUDA_HOME)]} {
	set cuda_lib_path "$env(CUDA_HOME)/lib64"
	set cudartLib "$cuda_lib_path/libcudart.so"
	set cufftLib "$cuda_lib_path/libcufft.so"
} else {
	set cudartLib ""
	set cufftLib ""
}

if {[info exist env(MKL_HOME)]} {
	set mkl_lib_path "$env(MKL_HOME)/lib/em64t"
	set mklcoreLib "$mkl_lib_path/libmkl_core.so"
	set mklintelLib "$mkl_lib_path/libmkl_intel_ilp64.so"
	set mklthreadedLib "$mkl_lib_path/libmkl_intel_thread.so"
	set ompLib "$mkl_lib_path/libiomp5.so"
} else {
	set mklcoreLib ""
	set mklintelLib ""
	set mklthreadedLib ""
...
$config SetValue CUDA_LIB [list $cudartLib $cufftLib]
$config SetValue MKL_LIB [list $mklcoreLib $mklintelLib $mklthreadedLib]
$config SetValue OMP_LIB [list $ompLib]
